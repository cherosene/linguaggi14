...
% ho aggiunto riassuntini alle slide, e note a piè diapositiva
...
\begin{frame}
	...
	\end{block}
	Questa è importante, ma se non diciamo l'altra, si semplifica di molto: diventa l'inclusione di insiemi
\end{frame}
...
\begin{frame}
	...
	Nota: nel file Game Semantics di Martin Hyland (quello in cui c'era la strategia di pt fisso) c'è l'esatta descrizione di questo albero
...
\begin{frame}
	...
	\begin{itemize}
		\item $\sigma ; \tau$
		\item $id_{A\limp A}$ (la copy-cat)
		\item $Lapp:(A \limp B)\otimes A \limp B$
	\end{itemize}
