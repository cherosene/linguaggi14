...
\begin{frame}
	...
	\begin{itemize}
		...
		\item<2-> $\lambda_{A\otimes B}=\lambda_A \coprod \lambda_B$
		\item<3-> $P_{A\otimes B}$ sono tutte le partite $s$ tali che
		\begin{itemize}
			\item $s|_{M_A} \in P_A \wedge s|_{M_B} \in P_B$
			\item Per ogni domanda in $A$, la risposta deve essere in $A$; lo stesso con $B$
		\end{itemize}
		\item<4-> $s\approx_{A\otimes B} t \Leftrightarrow s|_A \approx_A t|_A \wedge s|_B \approx_B t|_B \wedge fst(s)=fst(t)$ 
	\end{itemize}
	
	\onslide<5->{
	\begin{block}{Proprietà}
		\begin{itemize}
			\item<6-> Il prodotto tensore è associativo
			\item<7-> Esiste un elemento neutro $I$, ossia il gioco vuoto
			\item<8-> \textcolor{red}{Solamente il giocatore $O$ può cambiare componente di gioco}
		\end{itemize}
...
\begin{frame}
 	...
 	\begin{itemize}
 		...
 		\item<9-> NON è una categoria cartesiana chiusa (mancano i prodotti)
 	\end{itemize}
...
In particolare:
	...
	\begin{itemize}
		...
		\item<10-> Dati due oggetti, esiste l'esponente 
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item<6-> Per ogni $f: A\rightarrow B$, per ogni gioco $C$, $\perp_{B,C} \circ f = \perp _{A,C}$
	\end{itemize}
	\onslide<7->{
...
\begin{frame}
	...
	\only<1->{	 \draw [fill=green, opacity=.5] (8.5,-3) rectangle (6.5,-1);
			 ...
			 \only<3-> {\node (mv_2) at (4.5,-.5) [minimum size=2pt, scale=1.5] {$*_1$};}
			 \only<4-> {\node (mv_3) at (4.5,-3.5) [minimum size=2pt, scale=1.5] {$n_1$};}
...
\begin{frame}
	...
	Definiamo il preordine $\lesssim_A$ sulle strategie del gioco $A$: $x \lesssim_A y \; \Leftrightarrow \; \forall \alpha: A\rightarrow \Sigma \; x;\alpha \preccurlyeq_\Sigma y;\alpha$
