...
\begin{frame}
	...
	Definiamo $K_!(\mathcal{G})$ la categoria di \emph{co-Kleisli} dei giochi rispetto a $!$; in particolare:
	\begin{itemize}
		%\item $K_!(\mathcal{G})_0 = \mathcal{G}_0$
		\item $K_!(\mathcal{G})_0$ sono i giochi
		%\item Dati due giochi $A,B$, $Mor_{K_!(\mathcal{G})}(A,B) = Mor_{\mathcal{G}}(!A,B)$
		\item Dati due giochi $A,B$, i morfismi $Mor_{K_!(\mathcal{G})}(A,B)$ sono $\{ \sigma \text{ strategia di } A \Rightarrow B \; \equiv \; !A\limp B \; | \; \sigma \approx \sigma \} / \approx$
